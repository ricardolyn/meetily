...
// SidebarItem is now defined in SidebarProvider and includes created_at /
// project_id so the sidebar can render a date prefix and filter by project.

/** Sentinel value for "show meetings with no project". */
const FILTER_NO_PROJECT = '__none__';
/** Sentinel value for "show all meetings regardless of project". */
const FILTER_ALL = '__all__';

/**
 * Render the meeting title with a `[YYYY-MM-DD HH:MM] ` prefix derived from
 * its created_at timestamp. Falls back to the bare title if no timestamp is
 * available or it parses to NaN. Strips an already-present "Meeting YYYY-..."
 * prefix to avoid double-printing the placeholder format.
 */
function formatMeetingTitle(item: SidebarItem): string {
  if (!item.created_at) return item.title;
  const d = new Date(item.created_at);
  if (Number.isNaN(d.getTime())) return item.title;
  const pad = (n: number) => n.toString().padStart(2, '0');
  const prefix = `[${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())} ${pad(d.getHours())}:${pad(d.getMinutes())}]`;
  // Avoid stacking prefixes — strip the auto-generated "Meeting YYYY-..." form
  // generated by useRecordingStart and any prior bracket prefix we may have
  // emitted ourselves.
  const cleaned = item.title
    .replace(/^\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}\]\s*/, '')
    .replace(/^Meeting \d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}$/, 'Meeting');
...
const Sidebar: React.FC = () => {
  ...
  // Project filter for the meeting list. FILTER_ALL = no filter; FILTER_NO_PROJECT
  // = only meetings without a project; <id> = only meetings in that project.
  const [meetingFilterProjectId, setMeetingFilterProjectId] = useState<string>(FILTER_ALL);
  ...
  // Apply the project filter to a flat children list. Used before the
  // search-based filtering below so the two stack cleanly.
  const applyProjectFilter = useCallback(
    (children: SidebarItem[]): SidebarItem[] => {
      if (meetingFilterProjectId === FILTER_ALL) return children;
      if (meetingFilterProjectId === FILTER_NO_PROJECT) {
        return children.filter(c => !c.project_id);
      }
      return children.filter(c => c.project_id === meetingFilterProjectId);
    },
    [meetingFilterProjectId]
  );

  // Combine search results with sidebar items
  const filteredSidebarItems = useMemo(() => {
    // Apply project filter to children of the "meetings" folder first.
    const projectFilteredItems = sidebarItems.map(folder => {
      if (folder.type === 'folder' && folder.id === 'meetings' && folder.children) {
        return { ...folder, children: applyProjectFilter(folder.children) };
      }
      return folder;
    });

    if (!searchQuery.trim()) return projectFilteredItems;
  ...
  }, [sidebarItems, searchQuery, searchResults, expandedFolders, applyProjectFilter]);
    ...
    return (
      <div key={item.id}>
            ...
            <div className="flex flex-col w-full">
              <div className="flex items-center w-full">
                ...
                <span className="flex-1 break-words">
                  {isMeetingItem ? formatMeetingTitle(item) : item.title}
                </span>
                {isMeetingItem && (
        ...
        {item.type === 'folder' && isExpanded && item.children && (
          <div className="ml-1">
            {/* Project filter chip — only on the Meeting Notes folder */}
            {item.id === 'meetings' && projects.length > 0 && (
              <div className="px-3 py-2">
                <Select value={meetingFilterProjectId} onValueChange={setMeetingFilterProjectId}>
